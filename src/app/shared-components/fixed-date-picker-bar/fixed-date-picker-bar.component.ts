...
export class FixedDatePickerBarComponent
  ...
  baseUrl = "https://admin.hqbeds.com.br/pt-br/hqb";
  ...
  hostels: Hostel[] = [
    { value: "GKXgNR2gYx", viewValue: "Manaus" },
    { value: "AD4yYkYlNd", viewValue: "Figueiredo" }
  ];

  constructor(_requestTranslateService: RequestTranslateService) {
    _requestTranslateService.changeEmitted$.subscribe(language => {
  ...
  ngOnInit() {
    if (this.shouldHideBookingBarOnTop) {
      this.datePickerHTMLElement.nativeElement.style.display = this
        .shouldHideBookingBarOnTop
        ? "none"
        : "initial";
    }
  }
  ...
  ngAfterViewInit() {
    this.bookingBarOffsetTop =
      this.datePickerHTMLElement.nativeElement.offsetTop +
      this.datePickerHTMLElement.nativeElement.offsetHeight;
  }

  ngAfterViewChecked() {
    this.bookingBarOffsetTeste =
      this.datePickerHTMLElement.nativeElement.offsetTop +
      this.datePickerHTMLElement.nativeElement.offsetHeight;

    if (this.bookingBarOffsetTeste > this.bookingBarOffsetTop) {
      this.bookingBarOffsetTop = this.bookingBarOffsetTeste;
    }
  }
  ...
  checkLayoutDispay(verticalOffset) {
    this.bookingBarFixed =
      verticalOffset > this.bookingBarOffsetTop ? true : false;
    this.hideDataPicker =
      this.bookingBarFixed && window.innerWidth < 768 ? true : false;
    this.shouldValidatInputDate =
      this.bookingBarFixed && window.innerWidth < 768 ? false : true;
    if (this.shouldHideBookingBarOnTop) {
      this.datePickerHTMLElement.nativeElement.style.display = this
        .bookingBarFixed
        ? "initial"
  ...
  onSelectDate(date) {
    ...
    if (this.arriveDateValue && this.departureDateValue) {
      this.dynamicLink = `${this.baseUrl}/${
        this.property === null ? "GKXgNR2gYx" : this.property
      }/availability?arrival=${this.arriveDateValue["year"]}-${
        this.arriveDateValue["month"]
      }-${this.arriveDateValue["day"]}&departure=${
        this.departureDateValue["year"]
      }-${this.departureDateValue["month"]}-${this.departureDateValue["day"]}`;
      this.buttonDisabled = false;
    }
  }

  redirectToHQSales() {
    if (!this.shouldValidatInputDate) {
      window.open(
        `${this.baseUrl}/${
          this.property === null ? "GKXgNR2gYx" : this.property
        }/availability`,
    ...
    if (this.arriveDateValue["day"] == undefined)
      return (this.arriveError = true);
    else this.arriveError = false;

    if (this.departureDateValue["day"] == undefined)
      return (this.departureError = true);
